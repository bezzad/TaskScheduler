    ...
    public static class JobsManager
    {
        ...
        public static void CheckupSetting(string settingPath, int nextCheckupInterval)
        {
            ...
            finally
            {
                if (settingChanged)
                {
                    RecurringJob.AddOrUpdate(() => CheckupSetting(settingPath, nextCheckupInterval), Cron.MinuteInterval(nextCheckupInterval), TimeZoneInfo.Local);
                }
            }
        }
            ...
            try
            {
                ...
                foreach (JObject j in setting.Jobs)
                {
                    try
                    {
                        ...
                        if (jobObj != null && jobObj.Notifications?.Any() != true && setting.Notifications?.Any() == true)
                        {
